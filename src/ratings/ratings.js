...
if (process.env.SERVICE_VERSION === 'v2') {
  if (process.env.DB_TYPE === 'mysql') {
    ...
    var redisHost = process.env.REDIS_DB_HOST
    var redisPort = process.env.REDIS_DB_PORT
...
dispatcher.onGet(/^\/ratings\/[0-9]*/, function (req, res) {
  ...
  if (Number.isNaN(productId)) {
    ...
    if (process.env.DB_TYPE === 'mysql') {
      ...
      redisClient.get('mykey', function (error, result) {
        if (error) {
          console.log(error)
          throw error
        }
      ...
      res.writeHead(200, {'Content-type': 'application/json'})
      res.end(JSON.stringify('hello from redis'))
    } else { // mongo db
      MongoClient.connect(url, function (err, client) {
        if (err) {
          res.writeHead(500, {'Content-type': 'application/json'})
          res.end(JSON.stringify({error: 'could not connect to ratings mongodb database url: '+url}))
          console.log(err)
        } else {
